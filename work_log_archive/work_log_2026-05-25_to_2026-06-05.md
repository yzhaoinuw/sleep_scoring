...
### Installation Packaging Experiment

- Created the `experiments/installation-packaging` branch for packaging and
  installation workflow work.
- Added a forward plan to `next_steps.md` that preserves the current small
  replacement-`app_src/` update workflow alongside full PyInstaller zip releases.
- Added first-pass Windows packaging files under `packaging/windows/`:
  dynamic PyInstaller spec, full app zip script, `app_src` update zip script,
  smoke checker, and brief usage notes.
- Added `run_desktop_app.py --smoke` so a packaged executable can verify it
  imports the side-by-side `app_src/` folder without opening the GUI.
- Changed the default full app package to exclude Torch while keeping
  sDREAMER code, non-Torch sDREAMER infrastructure, and checkpoint files in
  place.
  `app_src.inference` now imports sDREAMER modules lazily so the default
  stats-model backend can load without Torch.
- Added `release_artifacts/` to `.gitignore` for generated zips, manifests, and
  checksum files.
- Verification:
  - PowerShell parsed `make_full_app_zip.ps1`, `make_app_update_zip.ps1`, and
    `smoke_check_release.ps1`.
  - `C:\Users\yzhao\miniconda3\condabin\conda.bat run -n sleep_scoring_dash3.0 python -m py_compile packaging\windows\app.spec` passed.
  - `powershell -NoProfile -ExecutionPolicy Bypass -File .\packaging\windows\make_app_update_zip.ps1 -SkipTests -AllowDirty` produced `release_artifacts\sleep_scoring_app_v0.16.2-app_src_update.zip` and passed the app-src update smoke check.
  - Confirmed `sleep_scoring_dash3.0_dist` can report `PyInstaller 6.17.0` and import `app_src.VERSION` as `v0.16.2`.
  - `C:\Users\yzhao\miniconda3\condabin\conda.bat run -n sleep_scoring_dash3.0 python run_desktop_app.py --smoke` printed `Sleep Scoring App v0.16.2 smoke check OK`.
  - First full-build attempt exposed that PyInstaller spec files do not define
    `__file__`; fixed the spec to use `SLEEP_SCORING_REPO_ROOT` / current
    working directory for repo-root discovery.
  - `powershell -NoProfile -ExecutionPolicy Bypass -File .\packaging\windows\make_full_app_zip.ps1 -SkipTests -AllowDirty` produced `release_artifacts\sleep_scoring_app_v0.16.2-windows.zip`, passed release-folder smoke checks, and ran `run_desktop_app.exe --smoke` successfully. SHA256: `0560D7B3C67E783DEA1380CBE88B51EBD7988F1D3F06572D01DAC065BB5F5612`.
  - Initial pytest verification failed because `.pytest_tmp` had to exist before
    using the repo-local `--basetemp`; recreated `.pytest_tmp` and reran.
  - `C:\Users\yzhao\miniconda3\condabin\conda.bat run -n sleep_scoring_dash3.0 pytest --basetemp .pytest_tmp\installation_packaging -p no:cacheprovider` -> `66 passed, 1 warning`.
  - `C:\Users\yzhao\miniconda3\condabin\conda.bat run -n sleep_scoring_dash3.0 python -c "import sys; sys.modules['torch']=None; import app_src.inference; print('inference import without torch OK')"` passed.
  - An initial no-Torch rebuild over-excluded `timm`, `einops`, and
    `torchvision`; corrected the packaging rule so only the actual bundled
    Torch runtime is removed while the rest of the sDREAMER infrastructure
    remains available.
  - Rebuilt the corrected no-Torch default full app zip with `powershell -NoProfile -ExecutionPolicy Bypass -File .\packaging\windows\make_full_app_zip.ps1 -SkipTests -AllowDirty`; it passed release-folder smoke checks and `run_desktop_app.exe --smoke`. The zip size is about `176 MB` decimal / `168 MiB`. SHA256: `DD19C22162C01550DC8CE38B0DEF18280E0C7D06238A0336CCFE7F1A718FF100`.
  - After the user clarified that sDREAMER checkpoints should still ship,
    restored checkpoint inclusion and rebuilt the full zip. The package keeps
    `_internal/einops`, `_internal/timm`, `_internal/torchvision`,
    `models/sdreamer`, and both checkpoint files, while keeping `_internal/torch`
    out. Final SHA256: `DD19C22162C01550DC8CE38B0DEF18280E0C7D06238A0336CCFE7F1A718FF100`.
  - `C:\Users\yzhao\miniconda3\condabin\conda.bat run -n sleep_scoring_dash3.0 pytest --basetemp .pytest_tmp\installation_packaging -p no:cacheprovider` -> `66 passed, 1 warning`.
  - Rebuilt the `app_src` update zip after the lazy-import change with `powershell -NoProfile -ExecutionPolicy Bypass -File .\packaging\windows\make_app_update_zip.ps1 -SkipTests -AllowDirty`. SHA256: `63EDFB0BB9F8ADB46865BA9FC082D8F7B35EC4735FECF678EE2B079D25F0F5FD`.
- The full package still needs a manual double-click GUI launch before using
  this workflow for a shipped release.
...
- Apples-to-apples measurement against the 2026-05-25 Mac M4 baseline
  ...
  - Native Plotly release at `x_width=15295 s`: `dash_apply=379 ms`
    ...
    (`browser_total=251-259 ms`) â€” **+140 ms**.
  - Native Plotly release at `~65-500 s`: `dash_apply=314-347 ms`
    vs baseline ~190-205 ms â€” **+110-140 ms**.
...
- Apples-to-apples measurement vs the 2026-05-25 Mac M4 baseline:
  - Native Plotly release at `x_width=15295 s`: new browser_total 257 ms vs
    baseline 251-259 ms â€” within noise.
  - Native Plotly release at `x_width=376 s`: new browser_total 223 ms vs
    baseline 224-295 ms â€” at the low end of the baseline distribution but
    only one sample.
  - Keyboard at `x_width=376 s`: new browser_total 500-501 ms vs baseline
    480-549 ms â€” essentially identical.
  - Annotation auto-pan at `x_width=376 s`: new fetch 124-157 / apply
    ...
    145-180 / browser_total 254-330 ms â€” within roughly 10-15 ms, no
